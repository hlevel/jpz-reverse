...
def open_wechat_task_url(adb: AdbProtocol, url: str) -> None:
    """Ask Android to open a task/deep-link URL after WeChat is foreground."""
    if not url:
        raise ValueError("task URL is empty")
    # URL routing is delegated to Android/WeChat; no screen coordinates needed.
    adb._run(["shell", "am", "start", "-a", "android.intent.action.VIEW", "-d", url])
