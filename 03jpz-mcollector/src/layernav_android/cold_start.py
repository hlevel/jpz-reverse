...
def cold_start_app_from_launcher(
    ...
    *,
    ...
    if _try_monkey(adb, package):
        # WeChat can take much longer than a fixed 1.5s after force-stop.
        # Wait within the caller's total deadline before declaring this path lost.
        if _wait_for_foreground(adb, package, deadline):
...
def _wait_for_foreground(adb: AdbProtocol, package: str, deadline: float) -> bool:
    """Poll foreground state until the shared cold-start deadline expires."""
