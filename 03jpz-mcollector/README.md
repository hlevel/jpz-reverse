...
>
> **Python 3.12+** · 轻量依赖（仅 structlog） · MIT License
...
锁文件由 `uv lock` 生成并提交；日常更新依赖使用 `uv lock --upgrade`。
...
当前配置文件位于 `config/config.yaml`，包含设备、账号登录状态、任务接口和执行规则；本地配置文件已加入 `.gitignore`。

配置关联采用同名匹配：`devices[*].accounts.<task_name>` 对应 `tasks.<task_name>`，例如 `accounts.ctrip_mini_program` 对应 `tasks.ctrip_mini_program`。规则通过账号中的 `rule` 字段关联 `rules[].name`。
